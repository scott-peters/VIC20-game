...
JDA good thinking with the double write in a single loop
...
JDA can you arrange it such that your custom chars are in
JDA the correct place to begin with, avoiding the need to
JDA copy them?
CopyChars:
...
.copyloop:	
			lda		BITMAPS-1,y
			sta 	CUSTCHRS-1,y
			JDA glad to see the use of loops counting downwards
			dey
			bne		.copyloop
			rts
...
.DONE:		
			clc
			lda 	POSADDRHI
			JDA I think you can do this with xor and save an instruction

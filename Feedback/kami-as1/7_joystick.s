...
Keypress:
			...
			beq		Keypress
			JDA write these using assembler character consts (e.g., #'A)
			JDA to make the code more readable
			cmp		#$41				; If 'a' move left
			beq		Left
			cmp		#$44				; If 'd' move right
			beq		Right
			cmp		#$57				; If 'w' move up
			beq		Up
			cmp		#$53				; If 's' move down
			beq		Down
			JDA this case was already tested above
			cmp		#0
			bne		Keypress

Left:
			JDA the movement code has common code at both
			JDA the start and the end that could be factored out
			JDA into a single place
			...
			JDA why's this a jsr as opposed to a jmp?
			JDA this'll blow out your stack after a while
			jsr		Keypress
